  ...
  return std::visit(
      overloaded{
          [face_index, depth](const std::shared_ptr<BRep> &brep,
            ...
            face_center /= static_cast<float>(face_polygon.outer().size());
            ...
            auto transformation = pmp::look_at_matrix(face_center, look_at,
                                                      pmp::vec3(1.f, 0.f, 0.f));
            ...
            for (const auto &p : result) {
              ...
              // t_indices.insert(t_indices.end(), result.begin(),
              // result.end());
              ...
              std::vector<pmp::Vertex> top_face_vertices;
              // Add the top face
              for (const auto &pt : polygon->outer()) {
                auto pmp_pt = pmp::Point(pt.x(), pt.y(), depth);
                pmp_pt = pmp::affine_transform(transformation, pmp_pt);
                top_face_vertices.push_back(extruded.add_vertex(pmp_pt));
              }
              extruded.add_face(top_face_vertices);
            }

            // // for (auto &poly : polygon) {
            // std::vector<pmp::Vertex> new_face;
            // for (auto &pt : result[0].outer()) {
            //   auto pmp_pt = pmp::Point(pt.x(), pt.y(), 0.f);
            //   auto transformed_pt =
            //       pmp::affine_transform(transformation, pmp_pt);
            //   auto v = extruded.add_vertex(transformed_pt);
            //   new_face.push_back(v);
            // }
            // extruded.add_face(new_face);
            // //}

            // pmp::triangulate(extruded);
